...
public class TwitterController implements Controller {
  ...
  public Tweet postTweet(String[] args) {
    if (args.length !=3 ){
      throw new IllegalArgumentException("USAGE: TwitterCLIApp post \"tweet text\" \"latitude:longitude\"");
    }
    String text = args[1];
  ...
  public List<Tweet> deleteTweet(String[] args) {
    if (args.length !=2 ){
      throw new IllegalArgumentException("Invalid number of arguments. USAGE: TwitterCLIApp delete \"[id1,id2,..]\" ");
    }
    String ids = args[1];
